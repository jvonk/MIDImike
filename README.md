...
A video recording the Arduino MIDI output with SoundCloud. The clarinet excerpt is performed by the U.S. Army Field Band.
...
| `DST_TEXT` | Displays the output of the frequency and pitch detection module in a table format on the serial monitor and can be useful for verification.
...
- To measure the delay between the note being played and being recognized, I edited the code adding a timer to measure the delay between audio input and MIDI output.

Instead of taking input from the microphone, the audio samples are read from files on μSD card. The module sddir.cpp expects top level directories `notes` and `solos`. The next level of directories should have the name of the sample rate (e.g. 9615). From there it will recurse the subdirectories and open each `.wav` file.
...
To verify, I compared the generated MIDI piano roll to the original score, and listened to the synthesized MIDI events and compared them to the original performance.
