    ...
    for (uint_least8_t ii = 0; ii < max; ii++) {  // calculate symbol positions for later
        ...
        switch (ii) {
            case STAFFSYMBOL_NAME_FLAT:
                p->x.relative = true;
                p->x.value = -(_.noteRadius + FLAT_TO_NOTE_DISTANCE + width);
                p->y.relative = true;
                p->y.value = -(height / 2);
                break;
            case STAFFSYMBOL_NAME_TO_LOW:
                ...
                p->x.value = -(width / 2);
                p->y.relative = false;
                p->y.value = _.displaySize.y - _.bottom2loStaff / 4 - height / 2;
                break;
            case STAFFSYMBOL_NAME_TO_HIGH:
                p->x.relative = true;
                p->x.value = -(width * s->enlarge.x / 2);
                p->y.relative = false;
                p->y.value = _.top2hiStaff / 4 - height / 2;
                break;
            ...
            if (*b & mask) {
                for (uint_least8_t ye = 0; ye < s->enlarge.y; ye++) {
                    for (uint_least8_t xe = 0; xe < s->enlarge.x; xe++) {
                        _.tft->drawPixel(x + xx*s->enlarge.x + xe, y + yy*s->enlarge.y + ye, color);
                    }
                }
            }
            mask >>= 1;
