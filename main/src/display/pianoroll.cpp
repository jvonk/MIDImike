...
namespace {
	...
	segmentPitch_t const PITCH_MIN = Pitch::freq2pitch( CONFIG_MIDIMIKE_FREQ_MIN );
	segmentPitch_t const PITCH_MAX = Pitch::freq2pitch( CONFIG_MIDIMIKE_FREQ_MAX );
				 ...
				 SegmentBuf * const    segmentBuf )  // segment buffer containing notes
{
	...
	absoluteTime_t const drawInMsec = min( CONFIG_MIDIMIKE_MIN_SEGMENT_DURATION + maxLoopTime, (cursor - X_FIRSTNOTE)*_msecPerPixel );
